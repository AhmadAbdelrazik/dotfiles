...
- **`.config/hypr/`** — `hyprland.lua` (real entrypoint) + `hyprland.conf`, `hypridle.conf`, `hyprlock.conf`, `hyprpaper.conf`, `monitors.conf`, `workspaces.conf`
- **`.config/hyprlock/`** — `colors.conf` (Catppuccin palette) + `layout.conf` (wallpaper/clock/profile-pic layout)
...
- **`.config/walker/`** — app launcher `config.toml` + 22 CSS themes under `themes/`
- **`.config/yazi/`** — Catppuccin-mocha `theme.toml`
- **`.config/calcure/`** — `config.ini` + CSV data files (calendar/tasks/journal)
- **`.config/wlogout/`** — power menu `layout` + `style.css`
...
- **`.local/scripts/`** — `screenshot`, `screen-recorder-toggle`, `battery-notify`, `tmux-sessionizer`, `herdr-sessionizer`, `update-wallpaper` (on `$PATH`)
- **dotfiles** — `.zshrc`, `.bashrc`, `.tmux.conf`, `.gitignore`
...
.
├── .config/
│   ├── calcure/        # Terminal calendar & journal (config + data)
...
│   ├── hyprlock/       # Lock layout + colors
│   ├── kitty/          # Terminal config & themes
│   ├── walker/         # Application launcher (config + themes)
│   ├── waybar/         # Modular status bar (modules/ + tokens/)
│   ├── waypaper/       # Wallpaper GUI config
│   ├── wlogout/        # Power menu (layout + style)
...
- **JetBrainsMono Nerd Font** — required across Waybar, Kitty, Walker, Hyprlock
...
- **Lock screen** — single Hyprlock layout (`hyprlock.conf` → `colors.conf` + `layout.conf`) showing a blurred version of the current wallpaper, profile photo, clock/date, and keyboard layout.
- **Walker launcher** — powered by the Elephant data-provider service; prefixes for quickly scoping searches (`;` providers, `>` runner, `/` files, `.` symbols, `!` todo, `%` bookmarks, `=` calc, `@` web search, `:` clipboard, `$` windows).
...
- **Docker TUI** — Lazydocker in Kitty, bound to `$mod+Shift+D`.
- **Screen recording** — `screen-recorder-toggle` (bound to `$mod+Print`) records with `wf-recorder`, letting you choose PC/mic/no audio.
- **Theming** — Catppuccin Mocha throughout (Waybar, Kitty, Walker, Yazi, Tmux, Hyprlock, Calcure).
- **Battery notifications** — `battery-notify` monitors charge and warns at 25% / 10%.
